...
const exporter = async (records, fetchRelatedRecords) => {
	...
	let data = await Promise.all(records.map(async record => {
		const locations = (await axiosClient().get('locations', {
			params: {
				_id: { $in: record.locations }
			}
		})).data.data
		const notes = (await axiosClient().get('notes', {
			params: {
				_id: { $in: record.notes }
			}
		})).data.data
		...
		return {
			...
			artists_list: (record.artists && record.artists.length > 0) ? record.artists.map(artist => artists[artist] ? `${artists[artist].name.first} ${artists[artist].name.last}` : "").toString() : "",
			techniques_list: (record.techniques && record.techniques.length > 0) ? record.techniques.map(technique => techniques[technique] ? techniques[technique].name : "").toString() : "",
	...
	const headers = ['inventoryNumber', 'wvz', 'artists_list', 'title', 'height', 'width', 'depth', 'techniques_list', 'publishedYear', 'status', 'state', 'locations_list', 'sighted', 'notes_list', 'frameSize', 'insuranceValue', 'showHistory', 'literature', 'provenance', 'bio'] // order fields in the export
